...
layout(location = 3) out vec2 boundary;
...
void main() {
    ...
    float bInterpX = borderColor.w > 0 ? (inSize.x / (inSize.x - 2.0 * borderWidth)) : 1.0;
    float bInterpY = borderColor.w > 0 ? (inSize.y / (inSize.y - 2.0 * borderWidth)) : 1.0;
